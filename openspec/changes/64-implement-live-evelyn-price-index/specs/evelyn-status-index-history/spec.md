...
Each started Evelyn environment SHALL attempt its first sample immediately and subsequent samples approximately one minute after the preceding attempt completes. It SHALL add exactly one timestamped point per successful attempt and SHALL isolate a failed or temporarily unavailable Ticker lookup without adding a fake point, terminating Evelyn, or cancelling the next attempt.
